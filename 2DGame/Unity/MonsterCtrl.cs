...
public class MonsterCtrl : CharacterCtrl
{
    private float _moveSpeed = 3f;
    private float _traceDistance = 6f;
    ...
    private void MonsterBehaviour()
    {
        ...
        {
            _state = State.IDLE;
            _rigidbody2D.linearVelocity = Vector2.zero; // 이동 정지
            return;
        }
        ...
        switch (_state)
        {
            case State.TRACE:
                ...
                {
                    _rigidbody2D.linearVelocity = new Vector2(direction.x * _moveSpeed, 0);
                }
                break;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            _animator.SetTrigger(Define.onMonsterHit);
